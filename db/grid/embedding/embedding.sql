...
RETURNS SETOF grid.substation_cluster ROWS 1 AS $$
  select *
  from grid.substation_cluster
  where id = $1.substation_cluster_id
$$ STABLE LANGUAGE sql;
...
RETURNS SETOF grid.substation AS $$
  select *
  from grid.substation
  where substation_cluster_id = $1.id
$$ STABLE LANGUAGE sql;
...
RETURNS SETOF grid.line AS $$
  select *
  from grid.line
  where from_substation_cluster_id = $1.id
$$ STABLE LANGUAGE sql;
