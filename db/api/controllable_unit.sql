...
CREATE OR REPLACE VIEW controllable_unit_history WITH (
    ...
    SELECT
        ...
        cu.start_date,
        cu.status,
        cuap.accounting_point_system_operator_id
        AS connecting_system_operator_id,
        cu.recorded_by,
        lower(cu.record_time_range) AS recorded_at,
        null AS replaced_by,
        null AS replaced_at
    FROM flex.controllable_unit AS cu
        INNER JOIN flex.controllable_unit_accounting_point AS cuap
            ON cu.id = cuap.controllable_unit_id
    UNION ALL
    SELECT
        cu.history_id AS id,
        cu.id AS controllable_unit_id,
