...
-- changeset flex:api-current-controllable-unit-accounting-point-revoke runOnChange:false
REVOKE EXECUTE ON FUNCTION api.current_controllable_unit_accounting_point(
    text
) FROM public;

-- changeset flex:api-current-controllable-unit-accounting-point-grant runOnChange:true
GRANT EXECUTE ON FUNCTION api.current_controllable_unit_accounting_point(
    text
) TO flex_internal_data;

-- changeset flex:api-controllable-unit-lookup-check-end-user-matches-accounting-point-revoke runOnChange:false
REVOKE EXECUTE ON FUNCTION
api.controllable_unit_lookup_check_end_user_matches_accounting_point(text, text)
FROM public;

-- changeset flex:api-controllable-unit-lookup-check-end-user-matches-accounting-point-grant runOnChange:true
