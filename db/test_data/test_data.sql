...
-- changeset flex:test-data-generate-notices runOnChange:true endDelimiter:--
-- Generate test data that triggers various notice types when notice_sync() runs
CREATE OR REPLACE FUNCTION test_data.generate_notices()
RETURNS void
AS $$
DECLARE
  l_so_id bigint;
...
BEGIN
  ...
  -- Use Test Suite parties (account seq_id=1)
  SELECT id INTO l_so_id FROM flex.party WHERE name = 'Test SO';
  SELECT id INTO l_sp_id FROM flex.party WHERE name = 'Test SP';
  SELECT id INTO l_eu_id FROM flex.party WHERE name = 'Test EU';
  SELECT id INTO l_fiso_id FROM flex.party WHERE name = 'Test FISO';
  SELECT id INTO l_pt_id FROM flex.product_type WHERE business_id = 'manual_congestion';

  IF l_so_id IS NULL THEN
    RAISE NOTICE 'Test Suite parties not found, skipping notice generation';
    RETURN;
  END IF;

  --
  -- 1. SPPA with status 'requested' -> notice_sppa_status_requested
  --
  INSERT INTO flex.service_provider_product_application (
    service_provider_id, system_operator_id, product_type_ids
  ) VALUES (
    l_sp_id, l_so_id, ARRAY[l_pt_id]
  );

  --
  -- 2. SPG grid prequalification with status 'requested'
  --    -> notice_spggp_status_requested
  -- Use the existing Test SPG. The auto-created SPGGP is in 'in_progress',
  -- so we bypass the status trigger to reset it to 'requested'.
  --
  SELECT id INTO l_spg_id
  ...
  --
  -- 3. SPG product application with status 'requested'
  --    -> notice_spgpa_status_requested
  -- Reset the existing in_progress one back to requested
  --
  ...
  --
  -- 4. CU suspension on a non-active CU -> notice_cus_not_active
  --    Create a CU that stays in 'new' status, then suspend it directly
  --
  SELECT id INTO l_ap_id
  FROM flex.accounting_point
  LIMIT 1;

  INSERT INTO flex.controllable_unit (
    name, start_date, regulation_direction, maximum_active_power,
    accounting_point_id
  ) VALUES (
    'Notice Test CU Inactive', '2020-01-01', 'up', 1.0, l_ap_id
  ) RETURNING id INTO l_cu_id;

  -- CU needs a technical resource before activation
  INSERT INTO flex.technical_resource (
    name, controllable_unit_id, technology, maximum_active_power,
    device_type, make, model
  ) VALUES (
    'Notice Test TR', l_cu_id, ARRAY['ev_charging_device'], 1.0, 'other', 'ACME', 'Test'
  );

  -- activate then terminate so we can insert a suspension with a backdated
  -- record_time_range (the trigger checks status on insert for active CUs)
  UPDATE flex.controllable_unit SET status = 'active' WHERE id = l_cu_id;

  INSERT INTO flex.controllable_unit_suspension (
  ...
  --
  -- 5. CU suspension older than 2 weeks -> notice_cus_lingering
  --    Backdate the record_time_range and terminate the CU.
  --    Disable the trigger that checks CU is active on suspension updates.
  --
  ALTER TABLE flex.controllable_unit_suspension
  DISABLE TRIGGER controllable_unit_suspension_cu_status_insert;

  ALTER TABLE flex.controllable_unit_suspension
  DISABLE TRIGGER controllable_unit_suspension_audit_current;

  UPDATE flex.controllable_unit_suspension
  SET record_time_range = tstzrange(
    current_timestamp - INTERVAL '3 weeks',
    NULL, '[)'
  )
  WHERE controllable_unit_id = l_cu_id;

  -- now terminate the CU so the suspension is on a non-active CU
  -- (triggers notice_cus_not_active)
  UPDATE flex.controllable_unit SET status = 'terminated' WHERE id = l_cu_id;

  ALTER TABLE flex.controllable_unit_suspension
  ENABLE TRIGGER controllable_unit_suspension_cu_status_insert;

  ALTER TABLE flex.controllable_unit_suspension
  ENABLE TRIGGER controllable_unit_suspension_audit_current;

  --
  -- 6. Party staging mismatches -> notice_party_missing, _outdated, _residual
  --    fill_party_staging copies current parties, then we manipulate staging
  --
  PERFORM test_data.fill_party_staging();

  -- 6a. Missing party: add a staging record with a GLN not in flex.party
  --     Use the existing Test Suite entity org number so the org check passes
  INSERT INTO flex.party_staging (gln, org, name, type)
  VALUES (
    gs1.add_check_digit('708000000000'),
    (SELECT e.business_id FROM flex.entity e
     WHERE e.business_id_type = 'org' LIMIT 1),
    'Missing Test Party',
    'service_provider'
  )
  ON CONFLICT DO NOTHING;

  -- 6b. Outdated party: change the name in staging for an existing party
  UPDATE flex.party_staging
  SET name = name || ' UPDATED'
  WHERE gln = (
    SELECT business_id FROM flex.party
    WHERE name = 'Test SP' AND business_id_type = 'gln'
  );

  -- 6c. Residual party: remove a staging record so the party appears residual
  DELETE FROM flex.party_staging
  WHERE gln = (
    SELECT business_id FROM flex.party
    WHERE name = 'Test BRP' AND business_id_type = 'gln'
  );

  --
  -- 7. Sync notices so they appear in the notice table
  --
  PERFORM flex.notice_sync();
END;
$$ LANGUAGE plpgsql SECURITY DEFINER VOLATILE;
