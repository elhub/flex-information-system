...
-- RLS: APGL-FISO002
GRANT SELECT ON accounting_point_grid_location_history
TO flex_flexibility_information_system_operator;
CREATE POLICY "APGL_FISO002"
ON accounting_point_grid_location_history
FOR SELECT
TO flex_flexibility_information_system_operator
USING (true);
...
USING (
    ...
    EXISTS (
        SELECT 1
        FROM accounting_point_system_operator AS ap_so
        WHERE ap_so.accounting_point_id = accounting_point_grid_location_history.accounting_point_id -- noqa
            AND ap_so.system_operator_id = (SELECT current_party())
            AND ap_so.valid_time_range @> current_timestamp
    )
    OR
    ...
    EXISTS (
        ...
        WHERE
            spgpa.procuring_system_operator_id = (SELECT current_party())
            AND cu.accounting_point_id = accounting_point_grid_location_history.accounting_point_id -- noqa
    )
);
