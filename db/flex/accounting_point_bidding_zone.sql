...
WITH (security_invoker = false) AS (
    SELECT
        ...
        (
            ...
            * mga_bz.valid_time_range
        ) AS valid_time_range
    FROM flex.accounting_point_metering_grid_area AS ap_mga
        INNER JOIN flex.metering_grid_area_bidding_zone AS mga_bz
            ON ap_mga.metering_grid_area_id = mga_bz.metering_grid_area_id
                AND ap_mga.valid_time_range && mga_bz.valid_time_range
);
