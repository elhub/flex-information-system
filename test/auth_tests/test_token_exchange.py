...
def test_wrong_grant_type(sts):
    actor_token = sts.get_client(TestEntityClient.TEST).token
...
# A client restricted to a party can exchange its token for that same party
def test_party_restricted_client_ok(sts):
    entity_client = sts.get_client(TestEntityClient.TEST_SP)
    entity_token = entity_client.token
...
# A client restricted to a party cannot exchange its token for another party
def test_party_restricted_client_wrong_party(sts):
    entity_client = sts.get_client(TestEntityClient.TEST_SP)
    entity_token = entity_client.token
    so_id = sts.get_userinfo(sts.get_client(TestEntityClient.TEST, "SO"))["party_id"]
