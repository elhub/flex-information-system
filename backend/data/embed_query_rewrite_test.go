...
func TestEmbedQueryRewrite(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "example from function comment",
			input:      url.Values{"embed": {"related_table!(subrelation1,subrelation2!(subsubrelation))"}},
			wantSelect: "*,related_table!inner(*,subrelation1,subrelation2!inner(*,subsubrelation))",
			wantEmbed:  "",
			wantErr:    false,
		},
		{
		...
		{
			name:       "multiple top-level tables with sub-relations",
			input:      url.Values{"embed": {"table_a!(sub1),table_b!(sub2,sub3)"}},
			wantSelect: "*,table_a!inner(*,sub1),table_b!inner(*,sub2,sub3)",
			wantEmbed:  "",
			wantErr:    false,
		},
		{
			name:       "whitespace is stripped",
			input:      url.Values{"embed": {"related_table!( sub1, sub2 )"}},
			wantSelect: "*,related_table!inner(*,sub1,sub2)",
			wantEmbed:  "",
			wantErr:    false,
		},
		{
		...
		},
		{
			...
			input:      url.Values{"embed": {"table_1!(sub_2)"}},
			wantSelect: "*,table_1!inner(*,sub_2)",
			wantEmbed:  "",
			wantErr:    false,
		},
		{
		...
		{
			name:       "example from updated comment",
			input:      url.Values{"embed": {"related_table!(subrelation1!,subrelation2(subsubrelation))"}},
			wantSelect: "*,related_table!inner(*,subrelation1!inner,subrelation2(*,subsubrelation))",
			wantEmbed:  "",
			wantErr:    false,
		},
		{
