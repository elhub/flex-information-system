...
load: liquibase
    ...
    psql -X -v ON_ERROR_STOP=1 -d postgres -U postgres -f ./local/postgres/structure-data.sql
    psql -X -v ON_ERROR_STOP=1 -d postgres -U postgres -f ./local/postgres/users.sql
