...
-- changeset flex:{{ rel.child.resource }}-{{ rel.child.name }}-to-{{ rel.parent.resource }}-revoke runAlways:true endDelimiter:--
REVOKE EXECUTE ON FUNCTION
api.{{ rel.name }}(api.{{ rel.child.resource }})
FROM public;

-- changeset flex:{{ rel.child.resource }}-{{ rel.child.name }}-to-{{ rel.parent.resource }}-grant runAlways:true endDelimiter:--
