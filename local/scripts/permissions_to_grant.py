...
csv_iterator = csv.DictReader(sys.stdin, delimiter=";")
party_types = cast(Sequence[str], csv_iterator.fieldnames)[2:]
    ...
    for party_type in grants[resource].keys():
        for grant in grants[resource][party_type].keys():
            if "*" in grants[resource][party_type][grant]:
                ...
                fields = ",\n".join(
                    f"    {g}" for g in grants[resource][party_type][grant] if g != "*"
                )
                if len(fields) == 0:
