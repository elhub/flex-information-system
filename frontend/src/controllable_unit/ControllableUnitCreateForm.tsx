...
import { ReferenceInput, useNotify } from "react-admin";
...
import { format, formatISO, isPast, parse, parseISO } from "date-fns";

const zControllableUnitCreateForm = z.object({
    name: zControllableUnitCreateRequest.shape.name,
    regulation_direction: zControllableUnitCreateRequest.shape.regulation_direction,
    maximum_available_capacity: zControllableUnitCreateRequest.shape.maximum_available_capacity,
    accounting_point_id: zControllableUnitCreateRequest.shape.accounting_point_id,
    service_provider_id: zControllableUnitServiceProviderCreateRequest.shape.service_provider_id,
    end_user_id: zControllableUnitServiceProviderCreateRequest.shape.end_user_id,
    contract_reference: zControllableUnitServiceProviderCreateRequest.shape.contract_reference,
    start_date: zControllableUnitCreateRequest.shape.start_date.nonoptional().refine((date) => isPast(date), { message: "Start date must be in the future" }),
});
type ControllableUnitCreateFormValues = z.infer<typeof zControllableUnitCreateForm>;
...
export const ControllableUnitCreateForm = () => {
    ...
    const [savedControllableUnitId, setSavedControllableUnitId] = useState<number | undefined>(undefined);

    const { data: identity, isLoading: identityLoading } = useGetIdentity();
    if (identityLoading) return <>Loading...</>;

    const isServiceProvider = (identity as UserIdentity | undefined)?.role === "flex_service_provider";
    ...
    const record: Partial<ControllableUnitCreateFormValues> = {
        ...
        service_provider_id: isServiceProvider
            ...
            : undefined,
        start_date: controllableUnitOverride.start_date ? formatISO(parse(controllableUnitOverride.start_date, "yyyy-MM-dd", new Date())) : format(new Date(), "yyyy-MM-dd"),
    };
    ...
    const onSubmit = async (values: object) => {
        ...
        const controllableUnitServiceProviderData = zControllableUnitServiceProviderCreateRequest.parse({ ...values, controllable_unit_id: createdControllableUnit.data.id, valid_from: controllableUnitData.start_date });
        const createdControllableUnitServiceProvider = await createControllableUnitServiceProvider({
            body: {
                ...controllableUnitServiceProviderData,
                controllable_unit_id: createdControllableUnit.data.id,
                valid_from: controllableUnitData.start_date ? formatISO(parse(controllableUnitData.start_date, "yyyy-MM-dd", new Date())) : undefined,
            },
        });

        if (createdControllableUnitServiceProvider.error) {
            notify(createdControllableUnitServiceProvider.error.message, { type: "error" });
            ...
            <FormContainer>
                ...
                <div className="flex flex-col gap-3">
                    ...
                    <EnumInput
                        ...
                        disabled={!!savedControllableUnitId}
                    />
                    <TextInput {...fields.maximum_available_capacity} type="number" disabled={!!savedControllableUnitId} />
                </div>
                <Heading level={3} size="medium">
